...
def train():
    ...
    if not os.path.exists(PGN_DATASET):
        print("Dataset PGN não encontrado. Cancele e rode o script pra baixar o dataset primeiro.")
        sys.exit(1)
    ...
    for epoch in range(EPOCHS):
        ...
        for x_data, y_target in stream_pgn_batches(PGN_DATASET, BATCH_SIZE):
            ...
            if batches_processed % 50 == 0:
                print(f"  [Epoch {epoch+1}] Lote {batches_processed} treinado. Loss média: {total_loss/batches_processed:.4f}")
