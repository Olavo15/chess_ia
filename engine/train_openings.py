...
def stream_pgn_batches(pgn_path, batch_size=BATCH_SIZE, samples_per_game=20):
    """
    Lê o PGN e extrai APENAS posições do início do jogo (até o lance 15).
    """
...
def train_openings():
    print(f"Iniciando TREINO DE ABERTURA (Dataset: {PGN_DATASET})")
    print(f"Focando nos primeiros {MAX_OPENING_MOVE} lances...")

    if not os.path.exists(PGN_DATASET):
        print("Dataset PGN não encontrado.")
        sys.exit(1)
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"\n[Abertura] Iniciando Epoch {epoch+1}/{EPOCHS}...")
        ...
        for x_data, y_target in stream_pgn_batches(PGN_DATASET, BATCH_SIZE):
            ...
            if batches_processed % 10 == 0:
                print(
                    f"  [Lote {batches_processed}] Loss: {total_loss/batches_processed:.4f}"
                )

        if not os.path.exists("data"):
            os.makedirs("data")
        torch.save(model.state_dict(), MODEL_PATH)
        print(f"-> Epoch {epoch+1} finalizado. Pesos salvos.")
